...
- Refactored `ps` to enumerate processes with limited query access and reserve parent PID, command line, window title, and executable metadata for `-extended`, without WMI.
- Updated `ps` process responses and browser script to report an embedded certificate signer when available and send numeric `0` when the process start time is unavailable.

## [v2.5.2] - 2026-08-11

### Changed

- Fixed bug in updated `processes` information struct

## [v2.4.19] - 2026-07-10

### Changed

- Streamed upload and download file transfers in bounded chunks to reduce memory usage for large files

## [v2.4.18] - 2026-07-09

### Changed

- Load the user profile for `make_token` identities so user-scoped cryptographic operations work correctly under impersonation

## [v2.4.17] - 2026-07-08

### Changed

- Fixed bug in `builder.py` that would fallthrough specific encryption key dictionary parsing and leak keys in generic headers
...
- Merged PR #177 to fix bug with getsystem

## [v2.4.14] - 2026-04-28

### Changed

- Added more verbose errors to the upload command when there are pathing issues
- Added a fix to always uppercase the hostname from the environment variable for more stable comparisons

## [v2.4.13] - 2026-04-21
### Changed

- Merged in PR #175 for updating Apollo's use of impersonation tokens
- Updated the `COFFLoader.dll` to be compiled with mingw since the other version caused issues in some environments
